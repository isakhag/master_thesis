...
#%% Investigating the spatial distribution of different tree heights, diameters and basal areas to see if the trees are large in the area or small.
# Define file paths
...
# # Function to calculate area of bins
# def calculate_area_of_bins(raster, transform, bins):
#     pixel_size_x = transform[0]
#     pixel_size_y = -transform[4]  # Typically negative, so take absolute value
#     pixel_area = pixel_size_x * pixel_size_y
#     hist, bin_edges = np.histogram(raster, bins=bins)
#     bin_areas = hist * pixel_area / 1_000_000  # Convert to square kilometers
#     return bin_areas, bin_edges

# # Define bins for diameters, heights, and basal areas
# diam_bins = [0, 10, 20, 30, 40, 50, 60]
# hoyde_bins = [0, 5, 10, 15, 20, 25, 30]
# basal_bins = [0, 10, 20, 30, 40, 50, 60]

# # Calculate areas for each raster
# diam_areas, diam_edges = calculate_area_of_bins(raster_diam, transform_diam, diam_bins)
# hoyde_areas, hoyde_edges = calculate_area_of_bins(raster_hoyde, transform_hoyde, hoyde_bins)
# basal_areas, basal_edges = calculate_area_of_bins(raster_basal, transform_basal, basal_bins)

# # Function to plot the areas
# def plot_area_distribution(bin_edges, bin_areas, title, xlabel):
#     bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2
#     plt.figure(figsize=(10, 6))
#     plt.bar(bin_centers, bin_areas, width=np.diff(bin_edges), edgecolor='black', align='center')
#     plt.title(title)
#     plt.xlabel(xlabel)
...
# plot_area_distribution(diam_edges, diam_areas, 'Tree Diameter Distribution', 'Diameter (cm)')
# plot_area_distribution(hoyde_edges, hoyde_areas, 'Tree Height Distribution', 'Height (m)')
# plot_area_distribution(basal_edges, basal_areas, 'Basal Area Distribution', 'Basal Area (sq m/ha)')
