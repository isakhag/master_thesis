...
# #Define the paths to the Excel files and raster files
# excel_files = {
...
# # Iterate over each key-value pair in the excel_files dictionary
# for key, value in excel_files.items():
#     # Read the Excel file into a DataFrame
#     df = pd.read_excel(value)
#     # Add a new column to keep track of the date
#     df['Date'] = key
...
# for key, raster_path in raster_files.items():
#     with rasterio.open(raster_path) as src:
#         coords = [(x, y) for x, y in zip(concatenated_df['x'], concatenated_df['y'])]
#         values = [val[0] for val in src.sample(coords)]
#         # Create a DataFrame with these values and the key
#         temp_df = pd.DataFrame({key: values}, index=concatenated_df.index)
